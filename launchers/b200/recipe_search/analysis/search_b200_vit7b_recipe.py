...
DEFAULT_RECIPE_ENV_PATH = DEFAULT_OUTPUT_ROOT / "recipe.env"
...
def write_recipe_env(recipes: dict[str, Recipe], path: Path) -> None:
    lines = [
        ...
        "# Helper artifact only. B200 launchers use YAML configs as canonical recipes.",
    ]
...
def write_markdown(results: dict[str, Any], recipes: dict[str, Recipe], doc_path: Path, recipe_env_path: Path) -> None:
    lines = [
        ...
        f"- Generated helper env: `{recipe_env_path}`",
        "- B200 launchers do not source the helper env; canonical recipe values live in YAML configs.",
        "",
        "## Selected Recipes",
        "",
...
def main() -> int:
    ...
    print(f"Helper env: {args.recipe_env_path}")
    return 0
